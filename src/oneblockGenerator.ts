...
const Elements = {
    ...
    bottomTopColor: document.getElementById("bottom-top-color") as HTMLSelectElement,
    glassColor: document.getElementById("glass-color") as HTMLSelectElement,
    commandContainer: document.getElementById("command-container") as HTMLDivElement,
    addRowButton: document.getElementById("add-row") as HTMLButtonElement,
    generateButton: document.getElementById("generate") as HTMLButtonElement,
    includeSign: document.getElementById("include-sign") as HTMLInputElement
};
                        ...
                        Passengers: [
                            {
                                ...
                                Silent: 1,
                                Command: `/fill ~ ~3 ~3 ~2 ~3 ~${longestCommand + 4} ${Elements.bottomTopColor.value}_concrete`, // bottom
                                Passengers: [
                                    {
                                        id: "minecraft:command_block_minecart",
                                        Silent: 1,
                                                ...
                                                Passengers: [
                                                    {
                                                        id: "minecraft:command_block_minecart",
                                                        Silent: 1,
                                                        Command: `/fill ~ ~${rowAmount + 4} ~3 ~2 ~${rowAmount + 4} ~${longestCommand + 4} ${Elements.bottomTopColor.value}_concrete`, // top
                                                        Passengers: [
                                                            {
                                                                id: `minecraft:command_block_minecart`,
                                                                Silent: 1,
                                                                ...
                                                                Passengers: [
                                                                    {
                                                                        id: "minecraft:command_block_minecart",
                                                                        Silent: 1,
                                                                        Command: `/fill ~ ~4 ~${longestCommand + 4} ~2 ~${rowAmount + 3} ~${longestCommand + 4} ${
                                                                            Elements.glassColor.value
                                                                        }_stained_glass`,
                                                                        Passengers: [
                                                                            // {
                                                                            //     id: "minecraft:command_block_minecart",
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        for (var j = 0; j < row.commands.length; j++) {
            ...
            var json = {
                id: "minecraft:command_block_minecart",
                Silent: 1,
                Command: `/setblock ~1 ~${row.id + 4} ~${j + 4} ${command.type != CommandType.IMPULSE ? command.type + "_" : ""}command_block[conditional=${
                    command.conditional
                },facing=south]{Command:'${command.body}',auto:${command.activation === CommandActivation.ALWAYS_ACTIVE ? "1" : "0"}b}`
            };
...
function render() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        // add event listeners
        var addCommandButton = document.getElementById("add-command-" + i) as HTMLButtonElement;
        var removeRowButton = document.getElementById("remove-row-" + i) as HTMLButtonElement;
        ...
        for (var j = 0; j < row.commands.length; j++) {
            ...
            var removeCommandButton = document.getElementById("remove-command-" + commandId) as HTMLButtonElement;
            var commandType = document.getElementById("command-type-" + commandId) as HTMLSelectElement;
            var commandActivation = document.getElementById("command-activation-" + commandId) as HTMLSelectElement;
            var commandCondition = document.getElementById("command-conditional-" + commandId) as HTMLSelectElement;
            var commandBody = document.getElementById("command-text-" + commandId) as HTMLInputElement;

            if (removeCommandButton != null) removeCommandButton.addEventListener("click", () => row.removeCommand(command));
            if (commandType != null) {
...
class Row {
    ...
    removeCommandIndex(command: number) {
    ...
    toElement(): HTMLDivElement {
        ...
        const commandsHolder = elem.querySelector(`#commands-${this.id}`) as HTMLDivElement;
        for (var i = 0; i < this.commands.length; i++) {
...
class Command {
    parentRow: Row;
    id: number;
    type: CommandType = CommandType.IMPULSE;
    activation: CommandActivation = CommandActivation.POWER;
    conditional: boolean = false;
    body: string = "";

    constructor(row: Row, id: number) {
        this.parentRow = row;
        this.id = id;
    }

    updateCommandType(type: string) {
        this.type = CommandType[type.toUpperCase() as keyof typeof CommandType];
    }

    updateCommandActivation(activation: string) {
        this.activation = CommandActivation[activation.toUpperCase() as keyof typeof CommandActivation];
    }

    updateCommandConditional(conditional: string) {
        this.conditional = conditional == "true";
    }

    updateCommandBody(body: string) {
        this.body = body;
    }
