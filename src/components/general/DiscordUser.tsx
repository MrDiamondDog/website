...
export default function DiscordAuthBarrier({
    children,
    onUserChange,
    redirect,
    state,
}: React.PropsWithChildren & { onUserChange?: (user: APIUser) => void, redirect?: string, state?: string }) {
    ...
    return (<>
        <div className="flex flex-col gap-4">
            {(clientId && !user) &&
                <a href={authUrl(clientId, redirect, state)} className="no-style">
                    <Button className="flex flex-row gap-2"><FaDiscord /> Authorize With Discord</Button>
                </a>
            }
            ...
            {user && <div>
                <div className="flex flex-row gap-3 items-center bg-bg-lighter p-3 rounded-lg">
                    <img
                        ...
                        alt="Avatar"
                        className="rounded-full w-[50px] md:w-[75px]"
                    />
                    <div>
