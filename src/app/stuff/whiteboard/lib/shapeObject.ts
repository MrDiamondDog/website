...
export class ShapeObject extends CanvasObject {
    ...
    draw() {
        ...
        switch (this.strokeData.shape) {
            ...
            case "arrow":
                const angle = Math.atan2(this.strokeData.endY - this.strokeData.startY, this.strokeData.endX - this.strokeData.startX);
                const headLength = 10;

                ctx.moveTo(this.strokeData.startX, this.strokeData.startY);
                ctx.lineTo(this.strokeData.endX, this.strokeData.endY);

                ctx.moveTo(this.strokeData.endX - headLength * Math.cos(angle - Math.PI / 6), this.strokeData.endY - headLength * Math.sin(angle - Math.PI / 6));
                ctx.lineTo(this.strokeData.endX, this.strokeData.endY);
                ctx.lineTo(this.strokeData.endX - headLength * Math.cos(angle + Math.PI / 6), this.strokeData.endY - headLength * Math.sin(angle + Math.PI / 6));
